"""Parquet impl of ``EventsRepository``."""

from __future__ import annotations

from collections.abc import Iterable, Iterator
from dataclasses import asdict, fields
from pathlib import Path

import duckdb

from ..base import EventRow
from ._writer import write_table_part
from .schemas import EVENTS_SCHEMA_V1

_TABLE = "events"


class ParquetEventsRepository:
    def __init__(
        self,
        data_root: Path,
        *,
        compression: str = "zstd",
        row_group_size: int = 50_000,
    ) -> None:
        self._root = data_root
        self._compression = compression
        self._row_group_size = row_group_size

    def upsert_events(self, rows: Iterable[EventRow]) -> int:
        rows_list = list(rows)
        if not rows_list:
            return 0
        write_table_part(
            self._root, _TABLE, EVENTS_SCHEMA_V1,
            [asdict(r) for r in rows_list],
            compression=self._compression,
            row_group_size=self._row_group_size,
        )
        return len(rows_list)

    def _glob(self) -> str:
        return str(self._root / "normalised" / _TABLE / "dt=*" / "*.parquet")

    def _has_data(self) -> bool:
        return any((self._root / "normalised" / _TABLE).glob("dt=*/*.parquet"))

    def _row_from_dict(self, d: dict) -> EventRow:
        d = dict(d)
        d.pop("dt", None)
        names = {f.name for f in fields(EventRow)}
        return EventRow(**{k: v for k, v in d.items() if k in names})

    _LATEST_VIEW_SQL = (
        "WITH latest AS ("
        "  SELECT *, row_number() OVER (PARTITION BY id ORDER BY ingested_ts_ms DESC) AS rn"
        "  FROM read_parquet('{glob}', hive_partitioning=true)"
        ")"
    )

    def _query(self, sql: str, params: list) -> list[dict]:
        if not self._has_data():
            return []
        con = duckdb.connect()
        try:
            cur = con.execute(sql.format(glob=self._glob()), params)
            cols = [c[0] for c in cur.description]
            return [dict(zip(cols, r, strict=False)) for r in cur.fetchall()]
        finally:
            con.close()

    def get_event(self, event_id: str) -> EventRow | None:
        rows = self._query(
            self._LATEST_VIEW_SQL +
            " SELECT * EXCLUDE rn FROM latest WHERE rn = 1 AND id = ? LIMIT 1",
            [event_id],
        )
        return self._row_from_dict(rows[0]) if rows else None

    def iter_events(self) -> Iterator[EventRow]:
        rows = self._query(
            self._LATEST_VIEW_SQL +
            " SELECT * EXCLUDE rn FROM latest WHERE rn = 1",
            [],
        )
        for r in rows:
            yield self._row_from_dict(r)
