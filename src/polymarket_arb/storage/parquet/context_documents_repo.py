"""Parquet repository for context document snapshots."""
...
from ..base import ContextDocumentRow
from ._writer import write_table_part
from .schemas import CONTEXT_DOCUMENTS_SCHEMA_V1

_TABLE = "context_documents"


class ParquetContextDocumentsRepository:
    def __init__(
        self,
        data_root: Path,
        *,
    ...
    def _row_from_dict(self, d: dict) -> ContextDocumentRow:
        d = dict(d)
        d.pop("dt", None)
        names = {f.name for f in fields(ContextDocumentRow)}
        return ContextDocumentRow(**{k: v for k, v in d.items() if k in names})
    ...
    def get_latest(self, context_document_id: str) -> ContextDocumentRow | None:
        rows = self._query(
            ...
            + " AND context_document_id = ? LIMIT 1",
            [context_document_id],
        )
    ...
    def iter_latest(self) -> Iterator[ContextDocumentRow]:
        rows = self._query(
            self._LATEST
            + " SELECT * EXCLUDE rn FROM latest WHERE rn = 1"
            + " ORDER BY context_space_id, title",
            [],
        )
        for row in rows:
            yield self._row_from_dict(row)

    def iter_for_space(self, context_space_id: str) -> Iterator[ContextDocumentRow]:
        rows = self._query(
            self._LATEST
            + " SELECT * EXCLUDE rn FROM latest WHERE rn = 1"
            + " AND context_space_id = ? ORDER BY title",
            [context_space_id],
        )
        for row in rows:
            yield self._row_from_dict(row)
